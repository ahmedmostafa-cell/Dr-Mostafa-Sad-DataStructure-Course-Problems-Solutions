...
public:
	PhoneHashTable(int table_size , double loadfactor= 0.75) :
		table_size(table_size) , load_factor(loadfactor){
	...
	void rehashing() 
	{
		double check = (double)total_elements / table_size;
		if (check < load_factor)
			return;
		PhoneHashTable new_table( table_size*2,  load_factor);
